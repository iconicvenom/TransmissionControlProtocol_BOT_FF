...
def generate_random_name():
    ...
    name = "WOTAX" + "".join(random.choice(characters) for _ in range(6)).upper()
    return name
...
def generate_custom_password(random_length=9):
    ...
    return f"WOTAX-{random_part}-ULTIGER"
...
for i in range(50000):
    with open("BY Wotax gen.txt", "a") as file:
        try:
            r = create_acc(region)
            response = r["response"]
